"""Generate saliency maps for man/woman given the groundtruth captions"""
...
def main(_):
  ...
  # hack for the missing files
  if len(json_dict) < 500:    
    for entry in json_data['annotations']:
      image_id = entry['image_id']
      if image_id in json_dict: continue  # already added
      if str(image_id) not in image_ids: continue
      if image_id not in json_dict:
        caption = entry['caption']
  ...
  with tf.Session(graph=g) as sess:
    ...
    mask_dir_base = './data/mask-out-ims/%s_mask_size_%d_%s/' %(FLAGS.model_name, FLAGS.mask_size, osp.basename(FLAGS.img_path))
